...
//for GW as NODE AND SIM7080
    ...
    NodeUtilities_PrepareJsonAndSendToRoot(ubyCommand, uwValue, payload);
    vPortFree(payload);
    free(dataToSend);
...
#endif
#ifdef GATEWAY_ETH
        esp_err_t result = ETHROOT_Mainboard_OTA_Update_Data_Length_To_Be_Expected(total_expected_data, target_MCU_int, major_version, minor_version, ci_version, branch_id);
        if (result == ESP_OK)
        {
            OTA_overall_status = true;
...
#ifndef IPNODE
        // for OTA updates to a SIM7080, make sure the gW is not defined as a IPNODE+ SIM7080.
        // if it is defined as both, push the OTA updates through NODE
#ifdef GATEWAY_SIM7080
        esp_err_t result = SIM7080_Mainboard_OTA_Update_Data_Length_To_Be_Expected(total_expected_data, target_MCU_int, major_version, minor_version, ci_version, branch_id);
        if (result == ESP_OK)
        {
            OTA_overall_status = true;
            LED_assign_task(CNGW_LED_CMD_FW_UPDATE, CNGW_LED_CN);
