...
bool Utilities_StringToMacShortVersion(const char *strmac, uint8_t *mac)
{
	...
	{
		char substring[3];
		memcpy(substring, &strmac[i * 2], 2);
		substring[3] = '\0';
		if (1 != sscanf(substring, "%hhx", &mac[i]))
		{
			return false;
		}
	}
...
bool Utilities_ValidateHexString(char *macStr, int length)
{
	...
	if (number != 0 && errno == 0 && strlen(macStr) == length && (strcmp(endptr, "") == 0))
	{
		return true;
	}
	else
	{
		ESP_LOGI(TAG, "strlen of %d is invalid hexadecimal string of length %d", strlen(macStr), length);
		return false;
	}
}
...
bool Utilities_ValidateMacAddress(char *macAdd)
{
	...
	// strtok modifies original mac address, store it in a buffer
	if (!macAdd)
		return false;
	char *copy = (char *)malloc(strlen(macAdd) + 1);
	if (copy == NULL)
	{
		ESP_LOGE(TAG, "issue allocating memory in Utilities_ValidateMacAddress");
		return false;
	}
	strcpy(copy, macAdd);
...
// searches for the address *addr in *addrs_list. if found, removes it from addrs_list and decrements addrs_num
bool addrs_remove(uint8_t *addrs_list, size_t *addrs_num, const uint8_t *addr)
{
