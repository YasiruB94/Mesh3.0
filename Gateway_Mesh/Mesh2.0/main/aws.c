...
void AWS_CreateTopics()
{
    ...
    // Register log topic
    char registerDev[14] = "/registerDev/";
    registerLogTopic = (char *)realloc(registerLogTopic, (strlen(registerDev) + strlen(orgID) + 1) * sizeof(char));
    strcpy(registerLogTopic, orgID);
    strcat(registerLogTopic, registerDev);
...
void AWS_AWSTask()
{
    ...
    while (true)
    {
        ...
        if (NETWORK_ATTEMPTING_RECONNECT == rc || NETWORK_RECONNECTED == rc)
        {
            ...
            {
                mwifi_post_root_status(true);
                ESP_LOGE(TAG, "Reconnect attempt success!");
            }
        ...
        if (AWSPublishQueue != NULL && NETWORK_RECONNECT_TIMED_OUT_ERROR != rc && NETWORK_DISCONNECTED_ERROR != rc && NETWORK_ATTEMPTING_RECONNECT != rc)
        {
