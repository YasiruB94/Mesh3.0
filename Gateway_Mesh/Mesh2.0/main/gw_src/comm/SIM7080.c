...
bool SIM7080_connected_to_AWS = false;
...
esp_err_t init_SIM7080()
{
    ...
    // 4. create a transmit queue with maximum of 50 commands
    SIM7080_AWS_Tx_queue = xQueueCreate(50, sizeof(char*));
...
/**
 * @brief legacy code. Handles CENCE OTA FW when the device is a NODE. The commands comes from the ROOT and the NODE does the rest
 *  when working on this code, you may have to change the MeshStruct_t to NodeStruct_t
 ...
 */
bool SIM7080_UpgradeNodeCenceFirmware(MeshStruct_t *structRootWrite)
{
    ...
    status = GW_ROOT_prepare_memory_to_store_OTA_data_temporarily(SIM7080_OTA_data_recieved_length);
    if(status != ESP_OK)
    {
    ...
    // 7. copy the data from the SIM to the recently cleared memory in ESP32 (this includes the function GW_ROOT_process_OTA_packet)
...
#else
/**
 * @brief Handles CENCE OTA FW targertted to a GATEWAY_IPNODE
...
#endif
/**
 * @brief Handles CENCE OTA FW when the device is a ROOT+GATEWAY_SIM7080
 *  checks for FW validity, connects to URL and download the FW to SIM7080
 *  copies the FW to the ESP32 memory temporarily
 *  sends the FW to Mainboard
 * @param structRootWrite[in] URL to download the fW from
 */
bool SIM7080_UpgradeRootCenceFirmware(MeshStruct_t *structRootWrite)
{
    // 1. Initialize and reset variables
    esp_err_t status = ESP_FAIL;
    char * error_string = "none";
...
/**
 * @brief Legacy code, not used anymore. Gets the command from AWS for an OTA event through SIM7080
...
bool SIM7080_UpgradeRootESP32Firmware(MeshStruct_t *structRootWrite)
{
    // 0. Initialize and reset variables
    esp_err_t status = ESP_FAIL;
    esp_ota_handle_t ROOT_OTA_handle;
    char *error_string = "none";
    vTaskDelay(500 / portTICK_PERIOD_MS);
    ...
    // 2. check the version of the incoming FW
    ...
    // 6. clear the required memory size
    status = GW_ROOT_find_OTA_partition();
  
    if (status != ESP_OK)
    {
        error_string = "Failed to determine OTA partition";
        goto EXIT;
    }

    status = esp_ota_begin(ota_agent_core_update_partition, SIM7080_OTA_data_recieved_length, &ROOT_OTA_handle);
    if (status != ESP_OK)
    {
        error_string = "Failed to begin OTA";
        goto EXIT;
    }

    // 7. copy the data from the SIM to the recently cleared memory in ESP32 (this includes the function GW_ROOT_process_OTA_packet)
    status = SIM7080_copy_bytes_from_SIM_to_ESP(true, ROOT_OTA_handle);
    if(status != ESP_OK)
    {
        error_string = "Failed to copy bytes from SIM7080 to ESP32";
        goto EXIT;
    ...
    status = esp_ota_end(ROOT_OTA_handle);
    if(status != ESP_OK)
    {
        error_string = "Failed to end OTA";
        goto EXIT;
    }

    status = esp_ota_set_boot_partition(ota_agent_core_update_partition);
    if(status != ESP_OK)
    {
        error_string = "Failed to switch to the new OTA partition";
        goto EXIT;
    }
...
esp_err_t SIM7080_download_file_to_SIM_from_URL(char *url)
{
    ...
    status = send_msg_receive_polling((uint8_t *)command, strlen(command), 2, 180000);
    free(command);
 ...
 */
esp_err_t SIM7080_copy_bytes_from_SIM_to_ESP(bool ESP32_related, esp_ota_handle_t OTA_handler)
{
    ...
    while (current_file_pos < SIM7080_OTA_data_recieved_length)
    {
        ...
        {
            if (ESP32_related == false)
            {
                // FW is not intended for a ESP32 target. (target is CENCE) copy the recieved bytes to ESP32 memory
                GW_ROOT_process_OTA_packet(binary_chunk, packet_size);
            }
            else
            {
                // FW is targeted at ESP32.
                status = esp_ota_write(OTA_handler, binary_chunk, packet_size);
                ESP_LOGI(TAG, "\tData len: %d,\ttotal_received: %d,\twriting data status: %s", packet_size, current_file_pos, esp_err_to_name(status));
            }
...
esp_err_t SIM7080_msg_preprocessor(uint8_t *recvbuf)
{
        ...
        if (substring_start != NULL)
        {
            ...
            if (*substring_end == '}')
            {
                ...
                if (truncated_substring != NULL)
                {
                    ...
                    // it is a root. divert the incoming data to ROOT
                    if (xQueueSendToBack(rootCommandQueue, &truncated_substring, (TickType_t)0) != pdPASS)
                    {
                        ESP_LOGE(TAG, "Queue is full");
                        vPortFree(truncated_substring);
                    }
    ...
    else if(SIM7080_Strcmp_Data(recvbuf, GW_SIM7080.config.uart_buffer_size, "+SMSTATE: 0") == ESP_OK)
    {
        ...
        SIM7080_connected_to_AWS = false;
...
/**
 * @brief Legacy code, not used anymore. The intermediate function which is used to check for patterns in the incoming messages from SIM7080
 *  If the recieving message has a recognized pattern, then appropriate steps are taken accordingly
 * @return ESP_OK if there is no pattern found. ESP_FAIL if there is a pattern found
 * @note by setting temp_block_SIM7080_msg_preprocessor to true, you can skip any pattern checking for one run
 */
esp_err_t SIM7080_send_AWS_message(char *msg, int command_type)
{
    GW_SIM7080 = *SIM_Get_Context();
 ...
 * @brief The intermediate function which is used to check for patterns in the incoming messages from SIM7080
 *  If the recieving message has a recognized pattern, then appropriate steps are taken accordingly
 * The topic of the AWS publish is also appended to the message
 * @return ESP_OK if there is no pattern found. ESP_FAIL if there is a pattern found
 * @note by setting temp_block_SIM7080_msg_preprocessor to true, you can skip any pattern checking for one run
 */
esp_err_t SIM7080_send_AWS_message_as_ROOT(char *msg, char *topic)
{
    GW_SIM7080 = *SIM_Get_Context();
    ...
    if (command != NULL)
    {
        sprintf(command, "AT+SMPUB=\"%s\",%d,%d,%d", (uint8_t *)topic, message_size, AWS_PUB_QOS, AWS_PUB_RETAIN);
        if (send_msg_receive_polling((uint8_t *)command, (16 + strlen(topic) + snprintf(NULL, 0, "%d", message_size)), 1, 1000) == ESP_OK)
        {
            free(command);
            if (SIM7080_Strcmp_Data((uint8_t *)recieved_data_from_SIM7080, GW_SIM7080.config.uart_buffer_size, ">") == ESP_OK)
            {
 ...
 * AWS_DISCONNECT               : Disconnect from MQTT server
...
SIM7080_AWS_status get_SIM_connected_to_AWS()
{
    ...
    while (!exit_loop)
    {
        ...
        case AWS_DISCONNECT:
        {
            if (send_msg_receive_polling((uint8_t *)ATcmdC_SMDISC, strlen(ATcmdC_SMDISC), 1, 6000) == ESP_OK)
            {
                if (SIM7080_Strcmp_Data((uint8_t *)recieved_data_from_SIM7080, GW_SIM7080.config.uart_buffer_size, "OK") == ESP_OK)
                {
                    // MQTT disconnected, now reattempt connection
        ...
        case AWS_CHECK_MQTT_STATUS:
        {
            ...
            if (send_msg_receive_polling((uint8_t *)ATcmdQ_SMSTATE, strlen(ATcmdQ_SMSTATE), 1, 1000) == ESP_OK)
            {
                ...
                {
                    if (SIM7080_Strcmp_Data((uint8_t *)recieved_data_from_SIM7080, GW_SIM7080.config.uart_buffer_size, "+SMSTATE: 1") == ESP_OK)
                    {
                        // sometimes even if the MQTT status is connected, the device fails to subscribe to incoming mqtt messages.
                        // therefore, we need to first disconnect from the mqtt service and restart the process
                        session_instance = AWS_DISCONNECT;
                    }
                    else
                    {
        ...
        // if the session_instance is AWS_SESSION_SUCCESS or AWS_SESSION_FAILURE
        default:
        {
            exit_loop = true;
        }
