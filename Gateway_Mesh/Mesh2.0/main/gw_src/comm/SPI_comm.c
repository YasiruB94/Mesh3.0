...
StaticQueue_t GW_response_static_queue;
StaticQueue_t CN_message_static_queue;
StaticTask_t  xStaticTaskBuffer_GW_process_received_data; 
...
esp_err_t init_GW_SPI_communication()
{
    ...
    uint8_t  *psram_buffer_GW_response_queue = (uint8_t *) heap_caps_malloc(QUEUE_LENGTH * BUFFER * sizeof(uint8_t), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (psram_buffer_GW_response_queue == NULL)
    {
        return ESP_FAIL;
    }

    GW_response_queue = xQueueCreateStatic(QUEUE_LENGTH, BUFFER * sizeof(uint8_t), psram_buffer_GW_response_queue, &GW_response_static_queue);
    if (GW_response_queue == NULL)
    {
        return ESP_FAIL;
    }
    // initialize the CN_message queue
    uint8_t  *psram_buffer_CN_message_queue = (uint8_t *) heap_caps_malloc(QUEUE_LENGTH * BUFFER * sizeof(uint8_t), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (psram_buffer_CN_message_queue == NULL)
    {
        return ESP_FAIL;
    }

    CN_message_queue = xQueueCreateStatic(QUEUE_LENGTH, BUFFER * sizeof(uint8_t), psram_buffer_CN_message_queue, &CN_message_static_queue);
    if (CN_message_queue == NULL)
    {
        return ESP_FAIL;
    }
    ...
    if (result == ESP_OK)
    {
        // GW_Trancieve_Data must be in internal memory because spi_slave_queue_trans data must be in DMA-capable memory
         xTaskCreate(GW_Trancieve_Data, "GW_Trancieve_Data", 3000, NULL, 5, NULL);

         // Create memory space in PSRAM and assign this memory to a task to handle GW_process_received_data
         StackType_t *psram_buffer_GW_process_received_data_task = (StackType_t *)heap_caps_malloc(5000 * sizeof(StackType_t), MALLOC_CAP_SPIRAM);
         if (psram_buffer_GW_process_received_data_task == NULL)
         {
             return ESP_FAIL;
         }

         if (xTaskCreateStatic(GW_process_received_data, "GW_process_received_data", 5000, NULL, 5, psram_buffer_GW_process_received_data_task, &xStaticTaskBuffer_GW_process_received_data) == NULL)
         {
             return ESP_FAIL;
         }
