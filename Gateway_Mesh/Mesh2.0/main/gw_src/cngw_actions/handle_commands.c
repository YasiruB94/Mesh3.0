...
size_t Parse_1_Frame(uint8_t *packet, size_t dataSize)
{
    ...
    {   size_t next_buffer_pos = 1;
        ...
        {
            // if a message comes in the middle of an OTA FW update, ignore the message
                ...
                if (frame.message.command == CNGW_DEVINFO_CMD_Update)
                {
                    ...
                    if (message_crc8 == frame.message.crc || ignore_crc_check)
                    {
                        ...
                        case 0:
                            ...
                            char mac_string[27];
                            char final_string[38];
                            ESP_mac_to_string(cn_board_info.cn_mcu.serial, mac_string);
                            strcpy(final_string, "Connected To Mainboard ");
                            strcat(final_string, mac_string);
                            ...
                            Send_GW_message_to_AWS(60, 0, final_string);
                            break;
                        case 1:
                            // it is the sw mcu
